...
Connection.prototype.createCollection = _wrapConnHelper(
  function createCollection(collection, options, cb) {
...
Connection.prototype.startSession = _wrapConnHelper(function startSession(
  options,
  cb
) {
  if (typeof options === "function") {
...
Connection.prototype.dropCollection = _wrapConnHelper(function dropCollection(
  collection,
  cb
) {
...
Connection.prototype.openUri = function(uri, options, callback) {
  ...
  if (["string", "number"].indexOf(typeof options) !== -1) {
    throw new MongooseError(
      "Mongoose 5.x no longer supports " +
...
const handleUseMongoClient = function handleUseMongoClient(options) {
  console.warn(
    "WARNING: The `useMongoClient` option is no longer " +
...
Connection.prototype.model = function(name, schema, collection) {
  if (!(this instanceof Connection)) {
    throw new MongooseError(
      "`connection.model()` should not be run with " +
...
Connection.prototype.deleteModel = function(name) {
  if (typeof name === "string") {
    ...
    throw new Error(
      "First parameter to `deleteModel()` must be a string " +
        'or regexp, got "' +
        name +
        '"'
    );
...
Connection.prototype.authMechanismDoesNotRequirePassword = function() {
  if (this.options && this.options.auth) {
    return (
      noPasswordAuthMechanisms.indexOf(this.options.auth.authMechanism) >= 0
    );
  }
  return true;
};
